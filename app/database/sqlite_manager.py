...
class SQLiteManager:
    ...
    def save_literature_note(self, user_id: int, note_data: Dict[str, Any]) -> int:
        ...
        try:
            ...
            ''', (
                ...
                note_data.get('analysis_date', datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
    ...
    def delete_literature_note(self, user_id: int, note_id: int) -> bool:
        ...
        try:
            ...
            cursor.execute("DELETE FROM literature_notes WHERE id = ? AND user_id = ?", (note_id, user_id))
            conn.commit()
            conn.close()
    ...
    def save_academic_template(self, user_id: int, category: str, content: str) -> int:
        ...
        try:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?)
            ''', (user_id, category, content, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
